...
author: "**Federico Marini** (marinif@uni-mainz.de)</br>
  ...
  BIUM(MZ) - Core Facility Bioinformatics</br>
  </br>`r icon::fa('twitter')` `@FedeBioinfo`</br>"
...
`r icon::fa('twitter')` `@FedeBioinfo`

PhD in Biostatistics/Bioinformatics @IMBEI: **Development of applications for interactive and reproducible data analysis**

$\rightarrow$ Methods and tools to maximize information extraction, knowledge transfer, strengthen translational interactions 
    <!-- - Focus on gene regulation in platelets transcriptomics datasets -->
...
Enabling transparency, independent verification, truly *standing on the shoulder of giants*
...
**RNA-seq**: genes * samples tables

**Aim**: identification of differentially expressed genes, gene signatures, many more 
<!-- (Identification of cell subpopulations, description of developmental trajectories, dimension reduction) -->

<p align="center">
<img src="animation_1x3_resized_20secstart.gif" alt="" width="700"/>
</p>
...
<!-- **Differential Expression**: essential step to identify the molecular drivers of the observed phenotype -->
...
`r emo::ji("thanks")` The Bioconductor project makes these task possible for many researchers!    
...
- `pcaExplorer` (Bioc 3.3) - Interactive Visualization of RNA-seq Data Using a Principal Components Approach, `http://bioconductor.org/packages/pcaExplorer/` <!-- sample forensics & data-exploration-driven decisions -->
- `ideal` (Bioc 3.5) - Interactive Differential Expression AnaLysis, `http://bioconductor.org/packages/ideal/`
...
## Workflow/demo quick {.build}

<p align="center">
<img src="figures/SCHEMA_ideal.png" alt="" width="900"/>
</p>

Play `scfull_pcaexplorer_ideal.mov` (also at https://www.youtube.com/watch?v=EckimPqRcCg)
...
**scRNA-seq**: genes * cells -
**Aim**: Identification of cell subpopulations, description of developmental trajectories, dimension reduction

Keywords: *heterogeneity*, *reveal*, *subpopulations*...

Collection of tools for single cell RNA-seq: `https://www.scrna-tools.org/` (Seurat, monocle, scLVM, SCDE, scater, ...)


**Data exploration has probably never been so important** - hidden gems (hypotheses, findings) come out only via extensive (and proper) exploration



## Let's make this optimal {.build}

- this is a time consuming task
- often needed iterations and clever subsetting
...
library("iSEE")
iSEE(sce)
# couple of genes to check: 
## Zeisel, Science 2015   +   ## Tasic, Nature Neuroscience 2016
# Tbr1 (TF required for the final differentiation of cortical projection neurons);
# Thy1 (marker for the axonal processes of mature neurons (CD90));
# Snap25 (pan-neuronal); 
# Gad1 (pan-GABAergic); 
...
> - **real practical companions for any (RNA-seq) dataset**, fully integrated in Bioconductor

> - Final fellowship output focus on Thrombosis & Hemostasis, enabling analysis workflows of high-dimensional data: *platelet-o-pedia* portal as unique landing point, lower barriers to genomic data exploration
...
- **`http://bioconductor.org/packages/iSEE/`** (or try it out on our server)
- `http://bioconductor.org/packages/pcaExplorer/`
- `http://bioconductor.org/packages/ideal/`
- Shiny Server @IMBEI: </br>`http://shiny.imbei.uni-mainz.de:3838/[pcaExplorer|ideal|iSEE]` 
...
<code>marinif@uni-mainz.de</code> - `r icon::fa('twitter')` `@FedeBioinfo`
</p>
</div>
